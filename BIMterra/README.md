...
`frontend/` y `backend/` se despliegan como **dos proyectos de Vercel separados**
(no un monorepo con un solo proyecto), porque son cosas distintas: uno es un sitio
estático y el otro es una API con funciones serverless.

### Backend (API)

1. En Vercel, "Add New Project" → importa el repo → en "Root Directory" elige `backend`.
2. Vercel detecta `backend/api/index.js` automáticamente como función serverless.
   `backend/vercel.json` redirige todas las rutas (`/api/auth/login`,
   `/api/proyectos`, etc.) hacia esa función.
3. Variables de entorno en Vercel (Project Settings → Environment Variables):
   - `DATABASE_URL`: la cadena de conexión **con pooling** de tu proveedor de
     Postgres (Neon, Supabase, Railway). Sin pooling, cada invocación serverless
     abre una conexión nueva y agotas el límite de Postgres muy rápido.
   - `JWT_SECRET`
   - `JWT_EXPIRES_IN` (opcional, ej. `7d`)
   - `CORS_ORIGIN`: la URL del frontend ya desplegado (ej. `https://bimterra.vercel.app`)
4. `npm install` corre `postinstall` → `prisma generate` automáticamente, así el
   cliente de Prisma queda listo antes de que se empaqueten las funciones.
5. Las migraciones (`prisma migrate dev`) **no** se corren en Vercel — se corren
   una vez desde tu máquina apuntando a la base de datos de producción:
   ```bash
   cd backend
   DATABASE_URL="<tu-url-de-produccion>" npx prisma migrate deploy
   DATABASE_URL="<tu-url-de-produccion>" npm run prisma:seed   # opcional
   ```

### Frontend

1. "Add New Project" → mismo repo → "Root Directory" `frontend`. Vercel detecta
   Vite automáticamente (`npm run build`, carpeta `dist`).
2. Variable de entorno: `VITE_API_URL` apuntando a tu backend desplegado, ej.
   `https://bimterra-api.vercel.app/api`.
